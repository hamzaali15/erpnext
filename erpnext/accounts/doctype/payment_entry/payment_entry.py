...
class PaymentEntry(AccountsController):
            ...
            if d.reference_doctype not in valid_reference_doctypes:
                ...
                if not frappe.db.exists(d.reference_doctype, d.reference_name):
                    ...
                    if d.reference_doctype in frappe.get_hooks("invoice_doctypes"):
                        ...
                        if ref_doc.doctype == "Purchase Invoice" and ref_doc.get(
                            "on_hold"
                        ):
                            frappe.throw(
            ...
            if ref.payment_term and ref.reference_name:
                ...
                if not invoice_paid_amount_map.get(key):
                    ...
                    for term in payment_schedule:
                        ...
                        invoice_paid_amount_map[invoice_key]["outstanding"] = (
                            term.outstanding
                        )
                        invoice_paid_amount_map[invoice_key]["discounted_amt"] = (
                            ref.total_amount * (term.discount / 100)
                        )

        for idx, (key, allocated_amount) in enumerate(
            invoice_payment_amount_map.items(), 1
        ):
            if not invoice_paid_amount_map.get(key):
                frappe.throw(
            ...
            discounted_amt = flt(
                invoice_paid_amount_map.get(key, {}).get("discounted_amt")
            )
    ...
    def set_amounts_after_tax(self):
        ...
        for tax in self.get("taxes"):
            if not tax.included_in_paid_amount:
                ...
                base_amount = (
                    -1 * tax.base_tax_amount
                    if tax.add_deduct_tax == "Deduct"
                    else tax.base_tax_amount
                )
        ...
        self.paid_amount_after_tax = flt(
            flt(self.paid_amount) + flt(applicable_tax),
        ...
        self.received_amount_after_tax = flt(
            flt(self.received_amount) + flt(applicable_tax),
            self.precision("paid_amount_after_tax"),
        )
    ...
    def set_amounts_in_company_currency(self):
        self.base_paid_amount, self.base_received_amount, self.difference_amount = (
            0,
            0,
            0,
        )
        ...
        for d in self.get("references"):
            if d.allocated_amount:
                ...
                base_total_allocated_amount += flt(
                    flt(d.allocated_amount) * flt(d.exchange_rate),
                    self.precision("base_paid_amount"),
                )
        ...
        if self.party:
            ...
            if (
                ...
                self.unallocated_amount = (
                    ...
                    - (total_deductions + self.base_total_allocated_amount)
    ...
    def set_difference_amount(self):
        ...
        base_party_amount = flt(self.base_total_allocated_amount) + flt(
        ...
        if self.payment_type == "Receive":
            ...
            self.difference_amount = self.base_paid_amount - flt(
                self.base_received_amount
            )
        ...
        self.difference_amount = flt(
            self.difference_amount - total_deductions - included_taxes,
    ...
    def clear_unallocated_reference_document_rows(self):
        self.set(
            "references",
            self.get("references", {"allocated_amount": ["not in", [0, None, ""]]}),
        )
        frappe.db.sql(
    ...
    def update_advance_paid(self):
        if self.payment_type in ("Receive", "Pay") and self.party:
            for d in self.get("references"):
                if d.allocated_amount and d.reference_doctype in frappe.get_hooks(
    ...
    def get_exchange_rate(self):
        return (
            self.source_exchange_rate
            if self.payment_type == "Receive"
            else self.target_exchange_rate
        )
    ...
    def determine_exclusive_rate(self):
        ...
        self.paid_amount_after_tax = flt(
            self.paid_amount / (1 + cumulated_tax_fraction)
        )
    ...
    def calculate_taxes(self):
        ...
        for i, tax in enumerate(self.get("taxes")):
            ...
            if i == 0:
                tax.total = flt(
                    self.paid_amount_after_tax + current_tax_amount,
            ...
            if self.payment_type == "Pay":
                self.base_total_taxes_and_charges += flt(
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
    ...
    if invoice_ref_based_on_payment_terms:
        for idx, ref in invoice_ref_based_on_payment_terms.items():
            ...
            existing_row = list(
                filter(
                    lambda x: x.get("voucher_no") == voucher_no, outstanding_invoices
                )
            )
    ...
    if reference_doctype == "Dunning":
        ...
        if not total_amount:
            if party_account_currency == company_currency:
                ...
                total_amount = ref_doc.get("grand_total") or ref_doc.get(
                    "base_grand_total"
                )
                exchange_rate = 1
            else:
...
def set_party_account(dt, dn, doc, party_type):
    if dt == "Sales Invoice":
        party_account = (
            get_party_account_based_on_invoice_discounting(dn) or doc.debit_to
        )
...
def set_payment_type(dt, doc):
    if (
        dt == "Sales Order"
        or (dt in ("Sales Invoice", "Dunning") and doc.outstanding_amount > 0)
...
def set_grand_total_and_outstanding_amount(
    party_amount, dt, party_account_currency, doc
):
        ...
        for term in doc.payment_schedule:
            if (
                ...
                if term.discount_type == "Percentage":
                    discount_amount = flt(doc.get("grand_total")) * (
                        term.discount / 100
                    )
                else:
        ...
        for pe in self.references:
            if pe.reference_doctype == "Sales Invoice":
                ...
                if not frappe.db.exists(
                    "Purchase Invoice",
                    {"docstatus": 1, "custom_sales_invoice": pe.reference_name},
                ) and frappe.db.exists(
                    "Sales Invoice", {"docstatus": 1, "name": pe.reference_name}
                ):
                    si = frappe.get_doc(
                        "Sales Invoice", {"docstatus": 1, "name": pe.reference_name}
                    )
                    for itm in si.items:
                        suppliers_list = frappe.get_all(
                            "Suppliers Detail", {"disabled": 0}, pluck="supplier"
                        )
                        ...
                        if item.supplier_woo and item.supplier_woo in suppliers_list:
                            ...
                            supplier_index = next(
                                (
                                    i
                                    for i, d in enumerate(pi_dict)
                                    if item.supplier_woo in d
                                ),
                                None,
                            )
                            ...
                            if supplier_index is not None:
                                ...
                                pi_dict[supplier_index][item.supplier_woo].append(
                                    item.item_code
                                )
                            else:
                    ...
                    for row in pi_dict:
                        supplier = list(row.keys())[
                            0
                        ]  # Convert dict_keys to list and get the first (only) element
                        ...
                        for d in list(row.values())[0]:
                            pr_detail = frappe.db.sql(
                                """select pr.name as purchase_receipt, pr.posting_date, pri.purchase_order, poi.rate
								...
								where pr.docstatus=1 and pr.status='To Bill' and is_return=0 and pri.item_code='{0}' and pr.supplier='{1}'
                            ...
                            new_doc.append(
                                "items",
                                {
                                    ...
                                    "purchase_order": pr_detail[0].purchase_order
                                    if pr_detail
                                    else None,
                                    "purchase_receipt": pr_detail[0].purchase_receipt
                                    if pr_detail
                                    else None,
                                },
                            )
...
def cancel_purchase_invoice(self, method):
    if self.payment_type == "Receive":
        for pe in self.references:
            if pe.reference_doctype == "Sales Invoice" and frappe.db.exists(
                "Purchase Invoice",
                {"docstatus": 1, "custom_sales_invoice": pe.reference_name},
            ):
