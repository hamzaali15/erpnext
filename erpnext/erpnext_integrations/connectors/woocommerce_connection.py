...
def create_to_all_pi_api(**arg):
    ...
    ids_pr = frappe.db.get_list(
        "Purchase Receipt",
        pluck="s_code",
        filters={"s_code": ["like", productJson.get("id")]},
    )
    for id in ids_pr:
        if frappe.db.exists("Purchase Receipt", {"s_code": id, "docstatus": 1}):
            pr = frappe.get_doc("Purchase Receipt", {"s_code": id, "docstatus": 1})
            transaction_processing(
                [{"name": pr.name}], "Purchase Receipt", "Purchase Invoice"
            )


def create_to_all_pi(order_id):
    ids_pr = frappe.db.get_list(
        "Purchase Receipt", pluck="s_code", filters={"s_code": ["like", order_id]}
    )
    for id in ids_pr:
        if frappe.db.exists("Purchase Receipt", {"s_code": id, "docstatus": 1}):
            pr = frappe.get_doc("Purchase Receipt", {"s_code": id, "docstatus": 1})
...
def remove_all(order_id):
    ids = frappe.db.get_list(
        "Purchase Invoice", pluck="s_code", filters={"s_code": ["like", order_id]}
    )
    for id in ids:
        if frappe.db.exists("Purchase Invoice", {"s_code": id, "docstatus": 1}):
            purchase_invoice = frappe.get_doc(
                "Purchase Invoice", {"s_code": id, "docstatus": 1}
            )
    ...
    # done for pr
    # Start for po
    ids_po = frappe.db.get_list(
        "Purchase Order", pluck="s_code", filters={"s_code": ["like", order_id]}
    )
    for id in ids_po:
...
def updateProuduct(**arg):
    ...
    if frappe.db.exists("Item", {"woocommerce_id": productJson.get("id")}):
        ...
        if (
            productJson.get("categories") is not None
            and productJson.get("categories") != []
        ):
        ...
        if (
            productJson.get("variation") is not None
            and productJson.get("variation") != []
        ):
            for item in productJson.get("variation"):
                item_id = item.get("id")
                if frappe.db.exists("Item", {"woocommerce_id": item_id}):
                    variation = frappe.get_doc(
                        "Item", {"woocommerce_id": item.get("id")}
                    )
...
def intOrder(**arg):
    ...
    customer_name = (
        raw_billing_data.get("first_name") + " " + raw_billing_data.get("last_name")
    )
    ...
    create_sales_order(
        order, woocommerce_settings, customer_email, sys_lang, payment_method_title
    )
...
def updateOrder():
    ...
    if frappe.db.exists(
        ...
        order_pending = frappe.get_doc(
            "Sales Order", {"woocommerce_id": order.get("id"), "docstatus": 1}
        )
        if (
            order_pending.woocommerce_status == "pending"
            or order_pending.woocommerce_status == "error"
        ):
            return frappe.throw(
                title="Error",
                msg="Order still in pending status please make sure all products available",
    ...
    if order.get("status") == "out_for_delivery" or order.get("status") == "vanex":
        if frappe.db.exists("Delivery Note", {"po_no": order.get("id")}):
            order1 = frappe.get_doc(
                "Delivery Note", {"po_no": order.get("id"), "docstatus": 1}
            )
            transaction_processing(
...
def updateItems(order):
    try:
        if frappe.db.exists(
            ...
            deliveryl.save(True)
            if frappe.db.exists(
                "Payment Entry", {"reference_no": checkPaymentEntry, "docstatus": 1}
            ):
                paymentEntry = frappe.get_doc(
                    "Payment Entry", {"reference_no": checkPaymentEntry, "docstatus": 1}
                )
...
def afterProc():
    ...
    if (
        ...
        or order.get("status") == "vanex"
        or order.get("status") == "delivered"
        or order.get("status") == "user_returnd"
    ):
        if frappe.db.exists(
            "Delivery Note", {"po_no": order.get("id"), "docstatus": 1}
        ):
            ...
            transaction_processing(
                [{"name": order1.name}], "Sales Order", "Delivery Note"
            )
            frappe.db.commit()
            order2 = frappe.get_doc(
                "Delivery Note", {"po_no": order.get("id"), "docstatus": 1}
...
def addItem(data):
    if frappe.db.exists(
        "Sales Order", {"woocommerce_id": data.get("id"), "docstatus": 1}
    ):
        list = []
        all_dis = 0
        ...
        sys_lang = frappe.get_single("System Settings").language or "en"
        link_items_int(
            data.get("line_items"), woocommerce_settings, sys_lang, data.get("id")
        )
        ...
        if frappe.db.exists(
            "Sales Taxes and Charges", {"parent": order.name, "docstatus": 1}
        ):
            tax = frappe.get_doc(
                "Sales Taxes and Charges", {"parent": order.name, "docstatus": 1}
            )
...
def _order(*args, **kwargs):
    ...
    if event == "created":
        if not frappe.db.exists(
            ...
            raw_shipping_data = order.get("shipping")
            customer_name = (
                raw_billing_data.get("first_name")
                + " "
                + raw_billing_data.get("last_name")
            )
            payment_method_title = order.get("payment_method")
            ...
            link_items_int(
                order.get("line_items"), woocommerce_settings, sys_lang, order.get("id")
            )
            customer_email = raw_billing_data.get("email")
            ...
            update_sales_order(order, woocommerce_settings, customer_name, sys_lang)
            if (
                order.get("status") == "wc-processing"
            ...
            if order.get("status") == "cancelled" or order.get("status") == "refunded":
                if frappe.db.exists("Sales Invoice", {"po_no": order.get("id")}):
                    delivery = frappe.get_doc(
                        "Sales Invoice", {"po_no": order.get("id")}
                    )
                    if delivery.docstatus.is_submitted():
                        delivery.cancel()
                    if frappe.db.exists(
                    ...
                    if frappe.db.exists(
                        "Sales Order",
                        {"woocommerce_id": order.get("id"), "docstatus": 1},
                    ):
                        exists = frappe.get_doc(
                            "Sales Order",
                            {"woocommerce_id": order.get("id"), "docstatus": 1},
...
def check_mode_payment(name):
    if not frappe.db.exists("Mode of Payment", {"woocommerce_id": name.strip()}):
        ...
        mode_payment = frappe.get_doc(
            "Mode of Payment", {"woocommerce_id": name.strip()}
        )
        return mode_payment.name
...
def create_address(raw_data, customer, address_type):
    ...
    address_line1 = (
        (address_line1[:75] + "..") if len(address_line1) > 75 else address_line1
    )

    address.address_line1 = address_line1
    address_line2 = raw_data.get("address_2", "Not Provided")
    address.address_line2 = (
        (address_line2[:75] + "..") if len(address_line2) > 75 else address_line2
    )
...
def link_items_int(items_list, woocommerce_settings, sys_lang, orderId):
    ...
    for item_data in items_list:
        ...
        if item_data.get("seller_id") == 46840:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_shafaq.append(
                {
                    "item_code": item.item_code,
        ...
        if item_data.get("seller_id") == 46818:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_nasamat.append(
                {
                    "item_code": item.item_code,
        ...
        if item_data.get("seller_id") == 47268:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_rinas.append(
                {
                    "item_code": item.item_code,
        ...
        if item_data.get("seller_id") == 47323:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_rinasp.append(
                {
                    "item_code": item.item_code,
        ...
        if item_data.get("seller_id") == 48097:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_mfcphilips.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
        ...
        if item_data.get("seller_id") == 47554:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_alhashan.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
        ...
        if item_data.get("seller_id") == 47975:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_alfaelectric.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
        ...
        if item_data.get("seller_id") == 49755:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_danacosmetics.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
        ...
        if item_data.get("seller_id") == 50304:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_weqayamedicals.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
        ...
        if item_data.get("seller_id") == 50566:
            item = frappe.get_doc("Item", {"sku": item_woo_com_id})
            item_berlingerhaus.append(
                {
                    "item_code": item.item_code,
                    "qty": item_data.get("quantity"),
                    "rate": item_data.get("cost"),
    ...
    if item_shafaq is not None and item_shafaq != []:
        po = create_purchase_order(item_shafaq, "Alshafaq Alahmer", orderId)
    if item_nasamat is not None and item_nasamat != []:
        po = create_purchase_order(item_nasamat, "Nasamat", orderId)
    if item_rinas is not None and item_rinas != []:
        po = create_purchase_order(item_rinas, "Rinas", orderId)
    if item_rinasp is not None and item_rinasp != []:
        po = create_purchase_order(item_rinasp, "Rinas Perfumes", orderId)
    if item_alhashan is not None and item_alhashan != []:
        po = create_purchase_order(item_alhashan, "Alhashan", orderId)
    if item_mfcphilips is not None and item_mfcphilips != []:
        po = create_purchase_order(item_mfcphilips, "MFC Philips", orderId)
    if item_alfaelectric is not None and item_alfaelectric != []:
        po = create_purchase_order(item_alfaelectric, "Alfa Electric", orderId)
    if item_danacosmetics is not None and item_danacosmetics != []:
        po = create_purchase_order(item_danacosmetics, "Dana Cosmetics", orderId)
    if item_weqayamedicals is not None and item_weqayamedicals != []:
        po = create_purchase_order(item_weqayamedicals, "Weqaya Medicals", orderId)
    if item_berlingerhaus is not None and item_berlingerhaus != []:
        po = create_purchase_order(item_berlingerhaus, "Berlinger Haus", orderId)
...
def woo_cat_old(link, product):
    if link is not None and link != "":
        ...
        if len(category) > 1:
            ...
            items = [
                item for item, count in collections.Counter(liist).items() if count == 1
            ]
            if len(items) > 1:
            ...
            print(
                [
                    item
                    for item, count in collections.Counter(liist).items()
                    if count == 1
                ]
            )
            return name
...
def create_sales_order(
    ...
    if not frappe.db.exists(
        ...
        customer_exists = frappe.get_value(
            "Customer", {"woocommerce_email": customer_email}
        )
        if customer_exists:
        ...
        new_sales_order.naming_series = (
            woocommerce_settings.sales_order_series or "SO-WOO-"
        )
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
    ...
    if (
        not frappe.db.exists("Warehouse", default_warehouse)
    ...
    for item in order.get("line_items"):
        ...
        if frappe.db.exists("Item", {"sku": item_woo_com_id}):
            ...
            new_sales_order.create_order_log = (
                f"product not found sku:{item_woo_com_id}"
            )
        if disc > 0:
            all_dis = all_dis + disc
            # add_tax_details(
        ...
        if status == "payment_entry":
            ...
            for order in deserialized_data:
                try:
                    ...
                    frappe.db.commit()
                    counter = counter + 1
                    url = "https://wordpress-1204933-4913305.cloudwaysapps.com/wp-json/api/gibran_user/erp_sync_status"
                    status = "delivered"
...
def sync_order_status_pi(order_id):
    ...
    for id in ids:
        if frappe.db.exists(
            "Sales Order", {"woocommerce_id": id, "docstatus": 1}
        ) and frappe.db.exists("Purchase Invoice", {"s_code": id, "docstatus": 1}):
            order = frappe.get_doc(
                "Sales Order", {"woocommerce_id": id, "docstatus": 1}
            )
...
def after_sync(order_id, order_status, page=""):
    ...
    url = "https://wordpress-1204933-4913305.cloudwaysapps.com/wp-json/api/gibran_user/erpnext_sync_done"
    body = {
        "order_id": order_id,
    }
    while page == "":
...
def sync_item_group():
    ...
    if frappe.db.exists("Item Group", {"id_woocommerce": category.get("parent")}):
        parent = frappe.get_doc(
            "Item Group", {"id_woocommerce": category.get("parent")}
        )
...
def long_job(self):
    ...
    with open(
        "/cloudclusters/erpnext/frappe-bench/sites/default/" + self.import_file, "r"
    ) as file:
        ...
        for row in csvreader:
            item_list.append(
                {
                    "soldby": row[indexSold] if indexSold or indexSold == 0 else "",
                    "shipment_code": row[shipment_code_ind]
                    if shipment_code_ind or shipment_code_ind == 0
                    else "",
                    "Cost": row[indexcost] if indexcost or indexcost == 0 else "",
                    "category": row[indexCategories]
                    if indexCategories or indexCategories == 0
                    else "",
                    "supplier": row[indexVendor].strip().lower()
                    if indexVendor or indexVendor == 0
                    else "",
                    "name": row[indexName] if indexName or indexName == 0 else "",
                    "brand": row[indexBrand] if indexBrand or indexBrand == 0 else "",
                    "seller_sku": row[indexSellerSKU]
                    if indexSellerSKU or indexSellerSKU == 0
                    else "",
                    "color": row[indexColor] if indexColor or indexColor == 0 else "",
                    "size": row[indexSize] if indexSize or indexSize == 0 else "",
                    "_sku": row[indexSSKU] if indexSSKU or indexSSKU == 0 else "",
                    "Descreption": row[indexDescription]
                    if indexDescription or indexDescription == 0
                    else "",
                    "var_sku": row[indexVarSKU]
                    if indexVarSKU or indexVarSKU == 0
                    else "",
                    "Images": row[indexImages]
                    if indexImages or indexImages == 0
                    else "",
                    "qty": row[indexQuantity]
                    if indexQuantity or indexQuantity == 0
                    else "",
                }
            )
        for row in item_list:
            ...
            if sku != "":
                ...
                if row.get("size"):
                    isIn = frappe.db.sql(
                        f"""SELECT * FROM `tabItem Variant Attribute`  WHERE  `tabItem Variant Attribute`.`attribute` ='Size' AND `tabItem Variant Attribute`.`parent` ='{sku}';"""
                    )
                ...
                if row.get("supplier"):
                    ...
                    if frappe.db.exists("Supplier", {"id_woo": row.get("supplier")}):
                        supplier = frappe.get_doc(
                            "Supplier", {"id_woo": row.get("supplier")}
                        )
                        item.supplier_woo = supplier.name
                    ...
                    cost = frappe.get_doc(
                        "Cost Center", {"woocommerce_id": row.get("supplier")}
                    )
                    item.cost_center = cost.name
                item.flags.ignore_mandatory = True
                item.save(ignore_permissions=True)
                ...
                if row.get("var_sku"):
                    if not frappe.db.exists("Item", {"item_code": row.get("var_sku")}):
                        ...
                        if cost is not None:
                            ...
                            cost = frappe.get_doc(
                                "Cost Center", {"cost_center_name": supplier12}
                            )
                            vtem.cost_center = cost.name
...
def add_att_value(item_data, type):
    ...
    isIn = frappe.db.sql(
        f"""SELECT * FROM `tabItem Attribute Value` WHERE `tabItem Attribute Value`.`attribute_value` ='{item_data}'"""
    )
...
def set_new_attribute_values(item_attr, values):
    item_attr.append(
        "item_attribute_values", {"attribute_value": values, "abbr": values}
    )
    return item_attr
...
def woo_cat(link, product):
    try:
        ...
        if len(category) > 1:
                ...
                liist.append(
                    {
                        ...
                        "parent": 0
                        if counter == 0
                        else category[counter - 1].lower().strip(),
                        "name": i.lower().strip(),
                    }
                )
            lastCat = liist[-1]
            if frappe.db.exists(
                "Item Group", {"name_woocommerce": lastCat.get("parent")}
            ):
                paren = frappe.get_doc(
                    "Item Group", {"name_woocommerce": lastCat.get("parent")}
                )
                if frappe.db.exists(
                    "Item Group",
                    {
                        "name_woocommerce": lastCat.get("name"),
                        "parent_item_group": paren.name,
                    },
                ):
                    cate = frappe.get_doc(
                        "Item Group",
            ...
            if frappe.db.exists(
                "Item Group", {"name_woocommerce": lastCat.get("name")}
            ):
                cate = frappe.get_doc(
                    "Item Group", {"name_woocommerce": lastCat.get("name")}
                )
...
def sync_order_status():
    ...
    ids = frappe.db.get_list(
        "Purchase Invoice", pluck="s_code", filters={"s_code": ["like", "%1%"]}
    )
    for id in ids:
        if frappe.db.exists(
            "Sales Order", {"woocommerce_id": id, "docstatus": 1}
...
def sync_sales_invoice_status():
    ids = frappe.db.get_list(
        "Sales Invoice",
        pluck="po_no",
        filters={"delivery_driver": ["like", ""], "po_no": ["like", "%112%"]},
        ignore_permissions=True,
    )
    for id in ids:
        if frappe.db.exists(
            "Sales Order", {"woocommerce_id": id, "docstatus": 1}
        ) and frappe.db.exists("Sales Invoice", {"po_no": id, "docstatus": 1}):
            order = frappe.get_doc(
                "Sales Order", {"woocommerce_id": id, "docstatus": 1}
            )
...
def updateErrors():
    ...
    ids = frappe.db.get_list(
        ...
        filters={"woocommerce_status": ["like", "error"]},
    )
    for id in ids:
        if frappe.db.exists("Sales Order", {"woocommerce_id": id, "docstatus": 1}):
            myObj = {"id": id}
            json_data = requests.post(
                "https://wordpress-1204933-4913305.cloudwaysapps.com/wp-json/api/gibran_user/erpnext_sync_error",
                json=myObj,
...
def sync_cancelled_status():
    cancelled_ids = frappe.db.get_list(
        "Sales Order", filters={"docstatus": 2, "woocommerce_status": "error"}
    )
